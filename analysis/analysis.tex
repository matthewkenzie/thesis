...
\begin{itemize}
  ...
  \item Given there are several nuisance parameters $\theta$ for which this is done, the signal model Gaussian parameters then get reparametrised as follows;
    ...
    \end{flalign}
where $\hat{\mu}$ and $\hat{\sigma}$ represent the nominal fitted values of the Gaussian means and widths in the \MC and $\sigma_{\mathrm{xs}}$, $\mathcal{B}$ and $L$ represent the cross-section, branching ratio and luminosity respectively.
\end{itemize}
