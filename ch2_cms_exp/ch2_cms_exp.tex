...
\begin{figure}
  ...
  \caption[CMS ECAL schematic]{A schematic drawing of the \CMS \ECAL layout \refrqd.}
  \label{fig:cms_ecal}
\end{figure}
...
The location of a supercluster is determined as the energy weighted mean position of the crystals in the supercluster. This gives a position resolution which is much smaller than the size of an individual crystal (typically \red{average area of barrel crystal in \eta and \phi}). Values for an electron of \pT=35~\GeV in the \ECAL barrel, in the absence of pileup, are $\sigma(\eta)=1.0\times10^{-3}$, $\sigma(\phi)=1.6$~mrad.
...
A traditional approach to detector based particle physics is to consider objects we measure as opposed to underlying physics objects. These are often known as calorimeter objects, for example, a track, an electromagnetic shower or a jet. A more modern approach is to couple information in all of the subdetector systems together to reconstruct more physical objects. For example, a charged hadron will leave a track, deposit some energy in the \ECAL and deposit the rest of its energy in the \HCAL. This technique of reconstruction is known as particle flow \footnote{The offcial name inside \CMS is \acf(GED) although this is rarely used} and is particularly useful when considering jets. Whilst an electron, photon or muon are fairly unique looking signatures hadrons are often not. The abundant number of gluons and quarks produced in \LHC collisions hadronise via the strong interaction as they travel away from the interaction. As they have typically high momentum the hadronistation occurs in a collimated fashion leaving a signature of several tracks and a reasonably spread hadronic shower.  
